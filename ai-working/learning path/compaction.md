...
- [ ] **PRIORITY**: Download historical data from Hugging Face
  - **Source**: GotThatData/kraken-trading-data (https://huggingface.co/datasets/GotThatData/kraken-trading-data)
  - Symbols: ETH/USD, BTC/USD, and other pairs
  - Multiple timeframes available (1m, 5m, 1h, 1d)
  - Pre-cleaned professional dataset
  - Alternative: CCXT direct download (if specific data needed)
- [ ] Load and validate Hugging Face dataset
- [ ] Convert to Nautilus format (if needed)
...
### Data Quality
- [ ] Validate OHLCV data integrity (no gaps, no outliers)
...
### Current Issues
1. **CCXT Data Validation** (Medium Priority)
   - Issue: "high was < open" errors when converting CCXT OHLC to Nautilus Bars
   - Impact: Can't use CCXT data directly yet
   - Workaround: Using Nautilus test data for tutorials
   - Fix Plan: Create robust data cleaning pipeline

2. **No Production Infrastructure** (High Priority)
   - Issue: No database, caching, or monitoring
   - Impact: Can't store results or monitor performance
   - Fix Plan: Docker Compose setup this week
